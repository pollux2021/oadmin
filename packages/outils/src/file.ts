import { API_FILE_PREFIX, FILE_TYPE_CONFG } from '@oconfig/file'
...
export function getFileType(fileName?: string): OFile.FileType {
  ...
  const fileTypes = Object.keys(FILE_TYPE_CONFG) as OFile.FileType[]
  const suffixType = fileTypes.find((type) => {
    return FILE_TYPE_CONFG[type].accept.includes(suffix)
  })
  return suffixType || 'unknow'
}
