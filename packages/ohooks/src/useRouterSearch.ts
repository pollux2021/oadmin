...
export default function useRouterSearch() {
  ...
  const hasPermisson = (
    ...
    // 非严格模式则只要名称开头匹配则命中
    if (!isStrict && !allowPermisson && menuCodes && menuCodes.length > 0) {
      allowPermisson = !!menuCodes.find((name) => {
        return new RegExp(`^${routeName}.{0}`).test(name)
      })
    }

    return allowPermisson
  }

  const routersHasPermisson = computed(() => {
    return routes.filter((item) => {
      const { meta = {}, name } = item
      const { requiresAuth, canSearch } = meta as any
      return (
        canSearch &&
        hasPermisson(name as string, requiresAuth, serverMenu.value, true)
      )
    })
  })
  ...
  const getMenusByName = (name: string, allowEmpty?: boolean) => {
    ...
    routersHasPermisson.value.forEach((item) => {
